...
class OneVsOne extends Component {
	renderCommonElement() {
		...
		return (
			commonVideo.perfectList.map((item, index) => <p key={item}><Icon onClick={this.props.setVideoOption} data-src={item} style={{ fontSize: '24px', verticalAlign: 'middle', marginRight: '6px' }} type="play-circle" theme="filled" />prefect {index + 1} {!item ? <span className="no-video">无视频</span> : ''}</p>)
		);
	}
