...
import {
	List, Pagination, Empty, Spin, BackTop,
} from 'antd';
...
class QuestionList extends Component {
	...
	constructor(props) {
		super(props);
		const { chapterStartTime, courseId } = this.props.match.params;
		const productId = this.props.match.params.chapterId;
		this.state = {
			currentPage: 0,
			chapterStartTime,
			courseId,
	...
	renderList() {
		...
		return this.props.quesList.map((item) => {
			const NowComponent = activityTypeElement[item.activityType];
			...
			<div className="question-list-wrapper">
				<Spin spinning={this.props.spinStatus} delay={500}>
					<div className="pagination-wrapper">
						<Pagination
							defaultCurrent={1}
							itemRender={QuestionList.itemRender}
							pageSize={20}
